...
type SpringUpdateRpcRecord = SpringInsertRpcRecord;
...
const toManagedSpring = (row: AdminManagementRowRecord): SpringRecord => ({
  accessNotes: row.access_notes,
...
const toManagedBrowseItem = (row: AdminManagementRowRecord): SpringBrowseItem => ({
  coverMedia: null,
  projection: {
    approvedReportCountConsidered: 0,
    confidence: row.confidence,
...
export class SupabaseSpringRepository implements SpringRepository {
  ...
  async getManagedById(springId: string) {
    ...
    return data ? toManagedBrowseItem(data as AdminManagementRowRecord) : null;
  }
  ...
  async listManaged(_cursor?: string | null, limit = 50): Promise<CursorPage<SpringBrowseItem>> {
    ...
    return {
      items: (data ?? []).map((row) => toManagedBrowseItem(row as AdminManagementRowRecord)),
      nextCursor: null,
    };
  }
  ...
  async update(command: UpdateSpringCommand) {
    ...
    return toCreatedSpring(data as SpringUpdateRpcRecord, command.location);
  }
